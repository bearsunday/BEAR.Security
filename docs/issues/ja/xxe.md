...
**注意:** PHP 8.0以降では、外部エンティティの読み込みはデフォルトで無効化されています。`LIBXML_NONET`はネットワークアクセスを防ぎますが、ローカルファイルへのアクセスには影響しません。完全な保護にはPHPのデフォルト動作に依存してください。`LIBXML_NONET`を明示することで意図を明確にできます。可能であればXMLの代わりにJSONを使用することを検討してください。
