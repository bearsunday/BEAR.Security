...
use BEAR\Security\Dast\DastScanner;
...
class DastScannerTest extends TestCase
{
    ...
    public function testRemoteFileInclusionPayloadHasPayloads(): void
    {
        $payload = new RemoteFileInclusionPayload();

        $this->assertNotEmpty($payload->getPayloads());
        $this->assertNotEmpty($payload->getSuccessPatterns());
        $this->assertSame('Remote File Inclusion (RFI)', $payload->getName());
        $this->assertSame(VulnerabilityInterface::SEVERITY_CRITICAL, $payload->getSeverity());
    }

    public function testCsrfPayloadHasPayloads(): void
    {
        $payload = new CsrfPayload();

        $this->assertNotEmpty($payload->getPayloads());
        $this->assertNotEmpty($payload->getSuccessPatterns());
        $this->assertSame('Cross-Site Request Forgery (CSRF)', $payload->getName());
        $this->assertSame(VulnerabilityInterface::SEVERITY_HIGH, $payload->getSeverity());
    }

    public function testResponseAnalyzerDetectsRfi(): void
    {
        $analyzer = new ResponseAnalyzer();
        $payload = new RemoteFileInclusionPayload();

        // Simulate phpinfo() output indicating RFI
        $responseBody = '<title>phpinfo()</title>PHP Version 8.1.0';
...
#1 /var/www/app/index.php(10): Controller->handle()';
    ...
    public function testDastScannerWithMockHttpClient(): void
    {
        ...
        $mockHttpClient = static function (string $url, string $method): array {
